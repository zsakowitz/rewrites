...
function toRuntime(ctx: Ctx, val: Val): string {
    ...
    switch (val.ty.k) {
        ...
        case T.Adt:
            return (val.ty.of as TyData[T.Adt]).adt.toRuntime(
                ctx,
                val as Val<T.Adt>,
            )!
        case T.Fn:
...
export const TARGET_JS = {
    ...
    symSplit(ctx, val) {
        ...
        if (tag) {
            ...
            return [
                new Val(text + ".x", Int, false),
                new Val(text + ".y", el, false),
            ]
        }
    },
    symJoin(ctx, tag, el) {
    ...
    arrayToCapped(_ctx, val, cap) {
        return val.transmute(
            new Ty(T.ArrayCapped, { el: val.ty.of.el, size: cap }),
        )
    },
    ...
    optFromVal(ctx, val) {
        ...
        if (val.const) {
            return new Val(
                { ok: true, value: val } satisfies Repr.Opt,
                ty,
                true,
            )
        } else if (val.ty.has1) {
