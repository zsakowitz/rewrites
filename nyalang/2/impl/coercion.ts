...
export class Coercions {
        ...
        switch (from.k) {
            ...
            case T.ArrayFixed: {
                ...
                if (into.is(T.ArrayUnsized)) {
                    return (
                        src.size.length == 1
                        && this.can(src.el, into.of.el, params)
                    )
                }
                return false
            }
            ...
            case T.Param:
                return (
                    into.is(T.Param)
                    && (into.of satisfies IdLabeled)
                        == (from.of as TyData[T.Param])
                )
            case T.Option:
    ...
    map(ctx: Ctx, val: Val, into: Ty, params: FnParams | null): Val {
        ...
        if (into.is(T.Option)) {
            ...
            return ctx.target.optFromVal(
                ctx,
                this.map(ctx, val, into.of, params),
            )
        }
        ...
        switch (from.k) {
            ...
            case T.Tuple: {
                ...
                const els2 = els.map((el, i) =>
                    this.map(ctx, el, dst[i]!, params),
                )
