...
export namespace Object {
    export type GetProperty<T, K extends PropertyKey> =
...
export namespace Uint {
    ...
    type ToArray<T extends number, R extends AsArray = []> =
    ...
    type SubtractArrays<A extends AsArray, B extends AsArray> =
        A extends [...B, ...infer U] ? U : []
    ...
    export type QuotientAndRemainder<A extends number, B extends number> =
        DivideArrays<ToArray<A>, ToArray<B>> extends (
            [infer U extends AsArray, infer V extends AsArray]
        ) ?
    ...
    export type Negate<T extends number> =
        T extends 0 ? 0
        : IsNegative<T> extends true ? Abs<T>
    ...
    export type IsLTE<A extends number, B extends number> =
        Subtract<A, B> extends 0 ? true : false
...
export namespace Int {
    ...
    type ToArray<T extends number> =
        T extends 0 ? [false, 0]
        : `${T}` extends `-${infer U extends number}` ? [true, U]
        : `${T}` extends `${infer U extends number}` ? [false, U]
        : never
    ...
    type ToNumber<T extends AsArray> =
        T[1] extends 0 ? 0
        : T[0] extends true ?
    ...
    type SignOfArray<T extends AsArray> =
        IsArrayZero<T> extends true ? [false, 0] : [T[0], 1]
    ...
    type AddArrays<A extends AsArray, B extends AsArray> =
        A[0] extends true ?
            B[0] extends true ?
    ...
    type IsLTEArray<A extends AsArray, B extends AsArray> =
        Boolean.And<IsArrayZero<A>, IsArrayZero<B>> extends true ?
            true // 0 <= 0
        : A[0] extends true ?
            B[0] extends true ?
...
export namespace Array {
    ...
    export type Reduce<T extends unknown[], I, F extends Function> =
        IsEmpty<T> extends true ? I
    ...
    export type AutoReduce<T extends unknown[], D, F extends Function> =
...
export namespace String {
    ...
    assert<IsEmpty<"">, true>(true)
    assert<IsEmpty<"sakawi">, false>(true)
    assert<IsEmpty<"zSnout">, false>(true)

    export type Head<T extends string> =
        T extends `${infer U}${string}` ? U : ""

    assert<Head<"sakawi">, "s">(true)
    ...
    assert<Tail<"sakawi">, "akawi">(true)
    assert<Tail<"zSnout">, "Snout">(true)
    assert<Tail<"">, "">(true)

    type _Length<T extends string, N extends number = 0> =
        T extends "" ? N : _Length<Tail<T>, number & Uint.Next<N>>

    export type Length<T extends string> = _Length<T>

    assert<Length<"sakawi">, 6>(true)
    assert<Length<"zSnout">, 6>(true)
    assert<Length<"">, 0>(true)

    export type SliceStart<T extends string, N extends number> =
        N extends 0 ? T
        : T extends `${string}${infer U}` ?
            SliceStart<U, number & Uint.Previous<N>>
        :   T

    assert<SliceStart<"sakawi", 2>, "kawi">(true)
    assert<SliceStart<"sakawi", 0>, "sakawi">(true)
    assert<SliceStart<"zSnout", 4>, "ut">(true)

    export type ToSliceIndex<T extends string, N extends number> =
        Uint.IsNegative<N> extends true ? Uint.Subtract<Length<T>, Uint.Abs<N>>
        :   N

    assert<ToSliceIndex<"sakawi", 4>, 4>(true)
    assert<ToSliceIndex<"sakawi", -4>, 2>(true)
    assert<ToSliceIndex<"zSnout", 0>, 0>(true)

    export type StartsWith<T extends string, U extends string> =
        T extends `${U}${string}` ? true : false

    assert<StartsWith<"sakawi", "s">, true>(true)
...
export namespace Decimal {
    ...
    type Length<T extends string> =
        ...
        : T extends (
            ...
            7
        : T extends `${number}${number}${number}${number}${number}${number}` ? 6
        : T extends `${number}${number}${number}${number}${number}` ? 5
        : T extends `${number}${number}${number}${number}` ? 4
        : T extends `${number}${number}${number}` ? 3
        : T extends `${number}${number}` ? 2
        : T extends `${number}` ? 1
        : 0
    ...
    type ToArray<T extends number> =
        `${T}` extends (
            `${infer WholePart extends number}.${infer Decimal extends string}`
        ) ?
            [
                `${WholePart}${Decimal}` extends (
                    `${infer Numerator extends number}`
                ) ?
                    Numerator
                :   never,
                Pow10<Length<Decimal>>,
            ]
        : `${T}` extends `${infer U extends number}` ? [U, 1]
        : never
