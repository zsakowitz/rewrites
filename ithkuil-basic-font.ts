...
export function convertPrimary(primary: PrimaryCharacter) {
    ...
    const bottomRight =
        PRIMARY_BOTTOM_RIGHT[primary.function || "STA"][
            primary.version || "PRC"
        ][primary.configuration?.startsWith("D") ? "D" : "M"][primary.stem ?? 1]
                ...
                value: output.value
                    .map((character) => {
                        ...
                        if (character.construct == Secondary) {
                            return convertSecondary(
                                character as SecondaryCharacter,
                            )
                        }

                        if (character.construct == Tertiary) {
                            return convertTertiary(
                                character as TertiaryCharacter,
                            )
                        }

                        if (character.construct == Quaternary) {
                            return convertQuaternary(
                                character as QuaternaryCharacter,
                            )
                        }

                        if (character.construct == Bias) {
                            return convertBias(
                                (character as BiasCharacter).bias,
                            )
                        }

                        if (character.construct == Register) {
                            return convertRegister(
                                character as RegisterCharacter,
                            )
                        }
