...
class Parser {
    ...
    expr(): ParseResult {
        ...
        return (
            (result = this.assignable()).ok ? result
            : (result = this.number()).ok ? result
            : (result = this.bool()).ok ? result
            : (result = this.block()).ok ? result
            : (result = this.ternary()).ok ? result
            : (result = this.assign()).ok ? result
            : (result = this.and()).ok ? result
            : (result = this.or()).ok ? result
            : (result = this.not()).ok ? result
            : (result = this.add()).ok ? result
            : (result = this.subtract()).ok ? result
            : (result = this.multiply()).ok ? result
            : (result = this.divide()).ok ? result
            : (result = this.mod()).ok ? result
            : (result = this.eq()).ok ? result
            : (result = this.lt()).ok ? result
            : (result = this.gt()).ok ? result
            : (result = this.void()).ok ? result
            : (result = this.text()).ok ? result
            : (result = this.call()).ok ? result
            : (result = this.spread()).ok ? result
            : (result = this.function()).ok ? result
            : (result = this.stdIn()).ok ? result
            : (result = this.stdOut()).ok ? result
    ...
    call() {
        return this.match("(")
            .map(() => this.expr())
            .map((target) =>
                this.exprs("comma").setJs(
                    (args) => `(${target.js}(${args.js}))`,
                ),
            )
    }
    ...
    function() {
        ...
        const snd = this.match("\\")
            .map(() => this.expr())
            .setJs(
                (expr) => `(${args.map((x) => x.js).join(", ")}) => ${expr.js}`,
            )
...
class ParseResult {
    ...
    /**
     * The name `lookahead` here is misleading, as this method consumes input. If
     * the "lookahead" was successful, this ParseResult is returned. Otherwise, a
     * failed ParseResult is returned.
     */
