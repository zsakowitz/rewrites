...
export class Scriptifier {
        ...
        switch (item.type) {
            ...
            case "fn": {
                if (item.args.length != args.length) {
                    throw new Error(
                        `${name} takes ${item.args.length} parameters.`,
                    )
                }

                if (item.args.some((x, i, a) => a.indexOf(x) != i)) {
                    throw new Error(
                        `${name} has multiple parameters with the same name.`,
                    )
                }
        ...
        for (const interp of interps) {
            if (typeof interp == "object") {
                for (const local in interp.locals) {
                    locals[local] = ScriptValue.local(
                        local,
                        interp.locals[local]!,
                    )
                }
    ...
    collate<const T extends readonly ScriptValue[]>(
        ...
        if (
            args.every(
                (x) => x.kind.list == null || typeof x.kind.list == "number",
            )
        ) {
            const size = args.reduce(
                (a, b) =>
                    typeof b.kind.list == "number" ?
                        Math.min(b.kind.list, a)
                    :   a,
                Infinity,
            )
...
return ${output};`,
        ...
        return this.callFn(name, args, (args) => {
            ...
            const size = ScriptValue.local(
                NameHasher.random("size"),
                new Kind("int"),
            )
            const value: ScriptValue = fn(
                args.map((arg) => {
