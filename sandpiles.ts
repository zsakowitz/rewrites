...
export class Sandpile extends Array<number[]> {
    ...
    constructor(numRows = 1, numCols = 1) {
        if (!numRows || !numCols) {
            throw new Error(
                "A sandpile must have positive and nonzero dimensions.",
            )
        }
    ...
    add(other: SandpileLike) {
        ...
        if (!Sandpile.isSandpileLike(other)) {
            throw new Error(
                "The item being added is not a proper SandpileLike.",
            )
        }

        if (other.length !== numRows || other[0]!.length !== numCols) {
            throw new Error(
                "When adding sandpiles, their dimensions must match.",
            )
        }
    ...
    ascii() {
        return this.map((row) =>
            row
                .map((cell) =>
                    (
                        Sandpile.characters[cell] ?? Sandpile.characters[4]!
                    ).repeat(2),
                )
                .join(""),
        ).join("\n")
