...
export function parseFormative(word: ParsedWord): PartialFormative {
    ...
    vr: {
        ...
        if (vr instanceof ConsonantForm) {
            throw new Error(
                "Expected Vr slot; found consonant form '" + vr + "'.",
            )
        }
    ...
    vncn: {
        ...
        if (cn == null) {
            if (caShortcut == "none") {
                throw new Error(
                    "Expected Ca, Cs, or Cn; found end of formative.",
                )
            } else {
    ...
    if (caShortcut != "none") {
        ...
        while (tokens.length) {
            ...
            affixes.push(
                parseAffix(
                    ...
                    cs.text,
                    affixes.length == 0
                        && (vx.hasGlottalStop || tokens.length == 0),
                ),
            )
        ...
        if (
            tokens.some(
                (token) =>
                    token instanceof ConsonantForm && token.isGeminated(),
            )
        ) {
            // Slot V: (CsVx...)
            // Slot VI: Ca
            ...
            while (tokens.length) {
                ...
                if (cs == null) {
                    throw new Error(
                        "Expected Cs or Ca; found end of formative.",
                    )
                }
                ...
                if (vx == null) {
                    throw new Error(
                        "Expected Vx to follow Cs; found end of formative.",
                    )
                }
    ...
    while (tokens.length) {
        ...
        slotVIIAffixes.push(
            parseAffix(
                vx,
                cs.text,
                slotVIIAffixes.length == 0 && tokens.length == 0,
            ),
        )
    }
