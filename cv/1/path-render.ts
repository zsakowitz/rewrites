...
const average = (a: number, b: number) => ((a + b) / 2).toFixed(20)
...
export function getPath(p: PointList, closed: boolean) {
    ...
    let result = `M${p[0]!.toFixed(20)},${p[1]!.toFixed(20)} Q${p[2]!.toFixed(20)},${p[3]!.toFixed(20)} ${average(p[2]!, p[4]!)},${average(p[3]!, p[5]!)} T`

    for (let i = 4, max = len - 2; i < max; i += 2) {
        result += `${average(p[i]!, p[i + 2]!)},${average(p[i + 1]!, p[i + 3]!)} `
    }

    result += `${p[p.length - 2]!.toFixed(20)},${p[p.length - 1]!.toFixed(20)} `
