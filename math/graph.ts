...
export class Visual<T, E> {
  readonly cv
  readonly #ctx
  readonly #pos = new WeakMap<Vertex<T, E>, Pos>()
  readonly #x = 0
  readonly #y = 0
  readonly #w = 20
  ...
  update(dt: number) {
    ...
    for (let i = 0; i < size; i++) {
      pos[i]![0] += forces[i]![0] * dt
      pos[i]![1] += forces[i]![1] * dt
    }
  }
}
...
const visual = new Visual(g)
document.body.appendChild(visual.cv)
