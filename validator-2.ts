...
export type Infer<T extends AnyValidator> =
...
> extends Validator<`${Start}${string}${End}`> {
    ...
    startsWith<T extends `${Start}${string}`>(
        text: T,
    ): StringValidator<T, End> {
        return this.refine(
            (data): data is `${T}${string}${End}` & typeof data =>
    ...
    endsWith<T extends `${string}${End}`>(text: T): StringValidator<Start, T> {
        return this.refine(
            (data): data is `${Start}${string}${T}` & typeof data =>
                data.endsWith(text),
    ...
    shape<U extends Record<string, AnyValidator>>(
        ...
        return this.chain<{ readonly [K in keyof U]: Infer<U[K]> }>((data) => {
            ...
            for (const key in shape) {
                const dataAtKey =
                    data[key as Extract<keyof U, string> & keyof T]
