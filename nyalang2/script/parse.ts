        ...
        switch (next?.k) {
            case K.NumOrTupleIndex: {
                ...
                if (!Number.isSafeInteger(idx)) {
                    next.issue(
                        `Bug: Tuple index '${next.content}' was not numeric.`,
                    )
                }
                ret = new Expr(join(ret.p, next.p), E.TupleIndex, {
                    on: ret,
                    idx,
                })
                break
    ...
    return (scan) => {
...

        {
            if (has(self, scan.peekK())) {
                scan.issue(
                    "Use parentheses to disambiguate operator precedence.",
                )
            }
        }
