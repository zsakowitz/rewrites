...
export namespace parse {
    type ReplaceLambdaWithBackslash<T extends string, H extends string = ""> =
        T extends `${infer A}λ${infer B}` ?
            ReplaceLambdaWithBackslash<B, `${H}${A}\\`>
        :   `${H}${T}`

    type RemoveComments<T extends string, H extends string = ""> =
        T extends (
            | `${infer A}#${string}\n${infer B}`
            | `${infer A}#${string}\r${infer B}`
        ) ?
            RemoveComments<B, `${H}${A}`>
        : T extends `${infer A}#${string}` ? A
        : `${H}${T}`
    ...
    export type PreLex<T extends string> =
        FindAndRemoveAliases<
    ...
    type MergeNameIntoBackslash<N extends string, R extends readonly Token[]> =
        N extends "" ? R
        : R extends (
            readonly [
    ...
    > =
        T extends `${infer Head}${infer Tail}` ?
            Split<
                ...
                readonly [
                    ...R,
                    { readonly type: "backslash"; readonly name: Head },
                ]
            >
        :   R
    ...
    > =
        ...
        : T extends `)${infer Rest}` ?
            ...
            >
        : T extends `.${infer Rest}` ?
            MergeNameIntoBackslash<N, R> extends (
                readonly [
            ...
            :   never
        : T extends `${infer Char}${infer Rest}` ? Lex<Rest, `${N}${Char}`, R>
        : MergeNameIntoBackslash<N, R>

    export namespace Lex {
        export type Debug<T extends readonly Token[], S extends string = ""> =
            T extends (
                readonly [
        ...
        while (index < length) {
            ...
            if (
                ...
                if (
                    ...
                    throw new Error(
                        "Found unexpected " + char + " after backslash.",
                    )
                }
    ...
    export namespace Tree {
        type ToArray<T extends readonly Tree[], S extends string = ""> =
            T extends (
                readonly [
        ...
        export type Debug<T extends Tree> =
            T extends (
                {
                    readonly type: "name"
                    readonly name: infer Name extends string
            ...
            : T extends readonly Tree[] ? ToArray<T>
            : never
    }
    ...
    > =
        T extends (
            ...
            Tail extends "paren" ? readonly [...Head, I]
            : Tail extends (
                {
                    readonly type: "backslash"
                    readonly name: infer Name extends string
                ...
                >
            : Tail extends Tree ? Fold<Head, readonly [Tail, ...I]>
            : never
        :   I
    ...
    function fold(
        ...
        for (let index = tree.length - 1; index >= 0; index--) {
            ...
            if (tail === "paren") {
                return head.concat(inserted) as [
                    ...typeof head,
                    typeof inserted,
                ]
            }
  ...
  > extends (infer U extends Tree)
    ...
    export function toTree(tokens: readonly Token[]): readonly Tree[] {
        return fold(
            toFlatTree(tokens) as readonly FlatTree[],
        ) as readonly Tree[]
    }
    ...
    type ArrayToNode<A extends readonly Tree[]> =
        A extends (
            readonly [
                infer Head extends Tree,
                ...infer Tail extends readonly Tree[],
    ...
    export type ToNode<T extends Tree> =
        T extends (
            {
                readonly type: "name"
                readonly name: infer Name extends string
        ...
        : T extends readonly Tree[] ? ArrayToNode<T>
        : never
...
export namespace Node {
    ...
    export type Rename<T extends Node, Old extends string, New extends string> =
    ...
    export type Includes<T extends readonly string[], U extends string> =
        T extends readonly [U, ...any] ? true
        : T extends readonly [any, ...infer Rest extends readonly string[]] ?
            Includes<Rest, U>
        :   false

    export type UniqueName<Avoid extends readonly string[]> =
        Includes<Avoid, "a"> extends false ? "a"
        : Includes<Avoid, "b"> extends false ? "b"
        : Includes<Avoid, "c"> extends false ? "c"
        : Includes<Avoid, "d"> extends false ? "d"
        : Includes<Avoid, "e"> extends false ? "e"
        : Includes<Avoid, "f"> extends false ? "f"
        : Includes<Avoid, "g"> extends false ? "g"
        : Includes<Avoid, "h"> extends false ? "h"
        : Includes<Avoid, "i"> extends false ? "i"
        : Includes<Avoid, "j"> extends false ? "j"
        : Includes<Avoid, "k"> extends false ? "k"
        : Includes<Avoid, "l"> extends false ? "l"
        : Includes<Avoid, "m"> extends false ? "m"
        : Includes<Avoid, "n"> extends false ? "n"
        : Includes<Avoid, "o"> extends false ? "o"
        : Includes<Avoid, "p"> extends false ? "p"
        : Includes<Avoid, "q"> extends false ? "q"
        : Includes<Avoid, "r"> extends false ? "r"
        : Includes<Avoid, "s"> extends false ? "s"
        : Includes<Avoid, "t"> extends false ? "t"
        : Includes<Avoid, "u"> extends false ? "u"
        : Includes<Avoid, "v"> extends false ? "v"
        : Includes<Avoid, "w"> extends false ? "w"
        : Includes<Avoid, "x"> extends false ? "x"
        : Includes<Avoid, "y"> extends false ? "y"
        : Includes<Avoid, "z"> extends false ? "z"
        : // @ts-ignore
            UniqueNameX<[0], Avoid>
    ...
    > =
        Includes<Avoid, `x${X["length"]}`> extends false ? `x${X["length"]}`
        :   // @ts-ignore
            UniqueNameX<[...X, 0], Avoid>

    export type Replace<T extends Node, Old extends string, New extends Node> =
        T extends Name<infer U> ?
            Equal<U, Old> extends true ?
                New
            :   Name<U>
        ...
        : T extends Lambda<infer U, infer B> ?
            Equal<U, Old> extends true ? Lambda<U, B>
            : IsFree<New, U> extends true ?
                // @ts-ignore
                Lambda<
                    // @ts-ignore
                    UniqueName<
                        readonly [T["name"], ...T["locals"], ...New["locals"]]
                    >,
                    Replace<
                        B,
                        Old,
    ...
    export type IsFree<T extends Node, N extends string> =
        T extends Name<infer U> ? Equal<N, U>
        : T extends Application<infer L extends Node, infer R extends Node> ?
            IsFree<L, N> extends true ? true
            : IsFree<R, N> extends true ? true
            : false
    ...
    type IsSingleCharacter<T extends string> =
    ...
    type SkipFirstCharacter<T extends string> =
        T extends `${string}${infer U}` ? U : ""
        ...
        replace(oldName: string, newNode: Node): Node {
            ...
            if (newNode.locals.includes(this.name)) {
                ...
                return new Lambda(
                    unique.name,
                    this.body
                        .rename(this.name, unique)
                        .replace(oldName, newNode),
                )
            }
    ...
    export class Application<L extends Node, R extends Node> extends Node {
        readonly canEval: L extends Lambda<string, Node> ? true
        : L["canEval"] extends true ? true
        : R["canEval"] extends true ? true
        : false
