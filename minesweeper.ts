...
type CellInfo =
  ...
  | { type: "number"; value: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8; status: CellStatus }
...
export class Game {
  ...
  getCellInfo(row: number, col: number): CellInfo | undefined {
    ...
    const status: CellStatus =
      cell & REVEALED ? "revealed" : cell & FLAG ? "flagged" : "unknown"
    ...
    for (let row = 0; row < this.rows; row++) {
      for (let col = 0; col < this.cols; col++) {
        ...
        if (cell.type == "mine") {
          ...
          } else if (this.hoveredRow == row && this.hoveredCol == col) {
  ...
  playAI() {
    ...
    for (let row = 0; row < this.rows; row++) {
      for (let col = 0; col < this.cols; col++) {
        ...
        if (info.type == "number") {
          const neighbors = this.neighbors(row, col).map(([row, col]) => ({
            row,
            col,
          ...
          if (
            ...
            info.value - flaggedNeighbors.length == unknownNeighbors.length
          ) {
            // mark all unknown neighbors
            this.mark(unknownNeighbors[0]!.row, unknownNeighbors[0]!.col)
          ...
          if (
            ...
            this.click(unknownNeighbors[0]!.row, unknownNeighbors[0]!.col)
